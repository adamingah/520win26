...
You should have Docker running on your computer by now. For the next several weeks we will continue to use the `eep520` image to start a container that has the gnu compiler tools, the Google test library, and the Doxygen API documentation tool. Please refer to the week 1 Readme file for complete Docker setup instructions. 

When you run the the container, you will initially be placed to the `/workspace` directory which is mounted the `/path/to/EE P520` directory in your host environment. This directory and its children are the only host directories available within the container environment.
